...
> ✅ **状态（2026-08-31）**：M1 骨架与 M2 控制按钮均已实现于 `tools/classisland_xingli_plugin/`（组件注册/2s 轮询只读显示 + 封面 + 上一首/播放·暂停/下一首按钮 → POST /control 回环），编译 **0 警告 0 错误**；并已补**设置配置化**（服务地址/轮询间隔可改，`AddComponent<T,TSettings>` 注册设置控件）与 **M3 异机部署文档半部**（§8，token 鉴权待排期）；ClassIsland 本体实机拖拽联调待验证。
...
> 前提：星璃 Windows/Android 端内置 NowPlaying 服务（端口 8742）；ClassIsland 插件设置页已支持自定义服务地址（2026-08-31）。

### 8.1 典型场景

| 场景 | 插件服务地址 | 展示 | 远程控制 |
|---|---|---|---|
| 同机（最常见） | `http://127.0.0.1:8742`（默认） | 完整（含本地封面直读） | ✅ 可用（回环） |
| 异机·局域网（星璃在另一台主机/手机） | `http://<星璃局域网IP>:8742` | 完整（封面 v1 不转发 → 占位图） | ❌ 服务端仅接受回环来源 |
| 异机·非局域网 | — | — | ❌（不建议；需 M3 token 鉴权打通公网） |
...
3. 插件设置页把「服务地址」改为 `http://<星璃局域网IP>:8742`，保存后组件自动重连。
4. 若跨网段/VLAN 不通：先同网段验证，再查防火墙与路由。

### 8.3 边界

- 封面：异机时 `coverUrl` 为本地路径不可达 → 插件按 http/https 判定隐藏（占位图），**不发起跨机文件读取**（v1 安全边界）。
- 控制：`POST /control` 服务端校验来源 IP 必须为本机回环，异机请求返回 403 `loopback only`；这是**有意的安全边界**（防局域网他人操控播放）。
- M3 剩余半部（可选 token 鉴权 + 公网可达）**未实现**，待用户确认是否进入排期。

---

*协议 v1 冻结于 M0；M1/M2 已交付；M3 异机部署文档随本稿，token 鉴权待排期。*