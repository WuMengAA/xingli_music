...
> ✅ **状态（2026-08-31）**：M1 骨架、M2 控制按钮、M3 均已实现于 `tools/classisland_xingli_plugin/`（组件注册/2s 轮询只读显示 + 封面 + 上一首/播放·暂停/下一首按钮 → POST /control 回环），编译 **0 警告 0 错误**；并已补**设置配置化**（服务地址/轮询间隔/token 可改，`AddComponent<T,TSettings>` 注册设置控件）与 **M3 可选 token 鉴权**（服务端 v1.1，单测 12/12 全绿；异机部署见 §8）；ClassIsland 本体实机拖拽联调待验证。
...
- [x] `test/services/now_playing_server_test.dart` 全绿（12/12，含 v1.1 token 鉴权用例）——已验证（2026-08-31），并入全量测试
...
- [ ] ClassIsland 插件在 ClassIsland 本体中拖入主界面，显示封面/标题/歌手/电台条（**M1 骨架已交付**：`tools/classisland_xingli_plugin/` 编译 0 警告 0 错误，组件注册/轮询/显示/封面已实现；实机联调待 ClassIsland 安装后验证）

---

## 8. 异机部署（M3）

> 前提：星璃 Windows/Android 端内置 NowPlaying 服务（端口 8742）；ClassIsland 插件设置页支持自定义服务地址 + 可选 token（2026-08-31，v1.1）。

### 8.1 典型场景

| 场景 | 插件服务地址 + token | 展示 | 远程控制 |
|---|---|---|---|
| 同机（最常见） | `http://127.0.0.1:8742`（默认，token 可留空） | 完整（含本地封面直读） | ✅ 可用（回环） |
| 异机·局域网·关闭鉴权 | `http://<星璃局域网IP>:8742` | 完整（封面 v1 不转发 → 占位图） | ❌ 服务端仅接受回环来源（默认） |
| 异机·局域网·启用 token | `http://<星璃局域网IP>:8742` + 插件填 token | 完整（封面同上） | ✅ 可用（token 即授权凭据） |
| 异机·非局域网 | 同上（需公网可达） | 完整 | ✅（token 保护下可控） |

> **鉴权开关（v1.1）**：星璃端服务构造传入 token 即启用——所有端点需 `?token=` 或 `Authorization: Bearer`，否则 401；此时 `/control` 放行任意来源（token 承担授权边界）。未设 token（默认）= v1 冻结行为（GET 局域网开放、/control 仅回环）。协议字段不变，纯新增鉴权层，**向后兼容 v1**。

### 8.2 异机配置步骤

1. 星璃端确认服务在跑：同机 `http://127.0.0.1:8742/health` 返回 `ok`；取本机局域网 IP（如 `ipconfig` / 手机设置页 IP）。
2. 星璃端放行防火墙：`8742` 端口允许入站（Windows 弹窗「允许访问」或入站规则 TCP 8742）。
3. **（可选）星璃端启用 token**：在服务构造传入 token（默认关闭）。启用后插件侧必须填一致 token，否则显示「未连接」（请求 401）。
4. 插件设置页把「服务地址」改为 `http://<星璃局域网IP>:8742`（需要控制时同页填 token），保存后组件自动重连。
5. 若跨网段/VLAN 不通：先同网段验证，再查防火墙与路由。

### 8.3 边界

- 封面：异机时 `coverUrl` 为本地路径不可达 → 插件按 http/https 判定隐藏（占位图），**不发起跨机文件读取**（v1 安全边界）。
- 控制：未启用 token 时 `POST /control` 服务端校验来源 IP 必须为本机回环，异机请求返回 403 `loopback only`——**有意的安全边界**（防局域网他人操控播放）。启用 token 后由鉴权承担授权边界（有 token 即可信）。
- 鉴权为「可选增强」：协议字段、路由、语义均未变，仅新增鉴权检查；插件与星璃任一未配置 token 时行为与 v1 完全一致。

---

*协议 v1 冻结于 M0；M1/M2 已交付；M3 已交付（异机部署文档 + 可选 token 鉴权，服务端单测 12/12 全绿、插件编译 0 警告 0 错误）。*