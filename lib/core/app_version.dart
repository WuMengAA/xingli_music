...
abstract final class AppVersion {
  ...
  ///   （仅 Alpha 渠道可见，如传感器调试）。
  /// cl02（09.13）：刮削器可用性加固 + 网易云歌单 / 搜索分页修复。
  /// ①刮削器：MusicBrainz 请求 UA 补联系方式（官方合规格式；缺联系方式的 UA
  ///   会被其 WAF 直接 403）、节流改**跨实例共享**（原先每进一次页面 new 一个
  ///   scraper，实例级节流等于没有；实测匿名搜索端点间隔 3s 连发仍会 503）、
  ///   加 20s 超时与 403/429/503 退避重试（2/4/6s）、错误文案透出服务端原话
  ///   并写日志，便于定位；
  /// ②网易云歌单：顶层歌单列表与歌单详情都补上分页——此前 provider 写死
  ///   limit=50/offset=0，详情页复用的共享薄壳又把加载源写死成「私人漫游」，
  ///   所以滚到底既不加载下一页、也刷不出新列表；现按 id/uri 去重追加 +
  ///   触底三态 + 下拉刷新，薄壳新增可注入的 loadMore 回调；
  /// ③聚合搜索：网易云（offset）与 B站（page）接上触底分页与下拉刷新，同词
...
const List<ChangelogEntry> changelog = <ChangelogEntry>[
  ...
  ChangelogEntry(
    version: '0.26.8.28',
    cl: 'beta_cl12',
    title: '搜索接入能力开关：网易云与 B站搜索正式开放',
    details: <String>[
      '网易云搜索、B站搜索正式开放。核对发现这两项客户端 8 月就已完整实现（含 B站 WBI 签名与播放地址解析），却在能力清单里误标为「未开放」，本次纠正——不是新做的功能，是此前没如实反映',
      'B站搜索不需要登录：实测 WBI 搜索免登录即可返回数据。登录影响的是播放音质（带 cookie 才有高码率音频流），所以清单里不标「需登录」，与网易云区分开',
      '聚合搜索页接入能力开关：在设置页关掉某个来源后，它的筛选标签消失、结果里不再出现、也不再发请求——关掉就是真的关掉',
      '关掉后不再催你登录：已关掉的来源不会再弹出「登录网易云」「登录哔哩哔哩」的提示',
      '「网易云 · 我的歌单」维持未开放：歌单链路确实还没落地，不提前宣称支持',
      '本次也修正了对项目现状的一次误判——曾以为 B站没有实现、计划走服务端代理，实为检索时结果被截断所致；改为沿用客户端已有实现，避免同一套逻辑维护两份',
    ],
  ),
  ChangelogEntry(
    version: '0.26.8.28',
    cl: 'beta_cl11',
    title: '内容来源可配置：能力层接入设置页与探索页',
    details: <String>[
      '网易云「每日推荐」正式开放：服务端把它翻为 ready，但实现仍在设备本机——不带 endpoint，登录态不出网，与「服务端不持账号资产」一致',
      '新增设置页「内容来源」：按服务端能力清单渲染开关，可裁剪探索页与曲库里出现的内容；关掉的项目不再发起任何请求',
      '区分「能力不可用」与「你关掉了」：没配本地目录、或服务端尚未开放的能力开关置灰不可点；只有真正能开关的才可切换，避免点了没反应',
      '探索页实验区叠加能力门控——在设置里关掉每日推荐，对应的实验卡片随即隐藏',
      '服务端不可用时不会误伤本机能力：清单拉不到（离线且无缓存）时放行，不让连不上服务端就把设备上的功能藏起来',
      '「网易云 · 搜索」与「我的歌单」如实标注为未开放：搜索与推荐客户端已具备，歌单链路尚未落地，不提前宣称支持',
    ],
  ),
  ChangelogEntry(
    version: '0.26.8.28',
