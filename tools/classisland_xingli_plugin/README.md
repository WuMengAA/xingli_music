...
- **服务地址**（默认 `http://127.0.0.1:8742`）——异机/容器场景改为局域网地址（注意 `/control` 未启用 token 时仅接受本机回环来源）
- **轮询间隔（秒）**（默认 2）——越小越跟手、越大越省电
- **鉴权 token（可选）**——星璃端启用 token 后此处需填一致值；留空 = 关闭鉴权（v1 冻结行为）
...
- **M1** ✅：组件骨架 + 只读显示（含封面）+ 设置配置化
- **M2** ✅：回环远程控制按钮（播放/暂停/切歌）
- **M3** ✅：异机部署（星璃仓库 `docs/方案_ClassIsland联动.md` §8）+ 可选 token 鉴权（服务端 v1.1，单测 12/12 全绿）
